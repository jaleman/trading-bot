...
if [[ -n "${PYTHON_BIN:-}" ]]; then
	...
	echo "Run $REPO_ROOT/scripts/bootstrap_trading_bot.sh before printing pending orders." >&2
	exit 1
fi

export PYTHONPATH="$APP_DIR/src${PYTHONPATH:+:$PYTHONPATH}"
cd "$APP_DIR"
exec "$RESOLVED_PYTHON_BIN" -m trading_bot.operator_commands pending-orders "$@"